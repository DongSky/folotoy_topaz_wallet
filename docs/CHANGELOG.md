...
- Package audited APK/firmware with checksums and data-preserving flash notes.
- Build and host checks, 62 Android unit tests and ten emulator tests passed.
  Physical-device and official WeChat acceptance remain pending for this build.
- Release publishing is manual; upstream synchronization has no schedule.

The following entries describe the inherited upstream baseline.
