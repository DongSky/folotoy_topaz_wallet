...
- 发布首个独立托帕钱包预览版，Android 为 0.3-dev（版本号 3）。
- 提供电子名片、外部支付宝／微信收款码导入、通过助记词导入钱包生成收款二维码及蓝牙页面同步。
- 更新应用及设备名片菜单名称，助记词改为普通明文输入。
- 打包已检查的 APK／固件、校验值与保留数据的刷写说明。
- 构建、主机检查、62 项 Android 单元测试和 10 项模拟器测试通过；本构建的实机与官方微信小程序验收仍待完成。
- 手动发布版本；上游同步取消定时执行。
